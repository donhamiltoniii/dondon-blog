...
describe('photo assets utilities', () => {
  ...
  describe('getImagesForPhotoAssetFromGlob', () => {
    it('should throw error when assetId is empty string', async () => {
      ...
      await expect(getImagesForPhotoAssetFromGlob('', {})).rejects.toThrow(
    ...
    it('should handle Promise rejection', async () => {
      ...
      const { getImagesForPhotoAssetFromGlob } = await import('./photos');
      await expect(getImagesForPhotoAssetFromGlob('error-image', mockGlob)).rejects.toThrow(
  ...
  describe('getPhotoAssets', () => {
    it('should fetch all photo assets from collection', async () => {
      const mockPhotos: CollectionEntry<'photos'>[] = [
        {
          id: 'photo1',
          collection: 'photos',
          data: {
      ...
      vi.mocked(getCollection).mockResolvedValue(mockPhotos);
