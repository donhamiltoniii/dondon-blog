...
export async function getImagesForPhotoAssetFromGlob(
  ...
  const matchingImagePaths = Object.entries(globResult).filter(([path]) =>
  ...
  const resolvedImages = await Promise.all(
    matchingImagePaths.map(([, imageFn]) =>
      imageFn().then((mod) => mod.default)
    )
  );
 ...
 * @returns Promise resolving to array of image metadata
 */
export async function getImagesForPhotoAsset(
  assetId: string
): Promise<ImageMetadata[]> {
